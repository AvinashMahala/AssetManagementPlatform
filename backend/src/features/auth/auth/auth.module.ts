...
export class AuthModule {
  ...
  private setupRoutes() {
    ...
    // Profile routes
    this.router.get('/profile', conditionalAuth(this.userService), async (req: AuthenticatedRequest, res: Response) => {
      try {
    ...
    this.router.put('/profile', conditionalAuth(this.userService), validateRequest.updateProfile, async (req: AuthenticatedRequest, res: Response) => {
      try {
        const userId = req.user?.id;
        if (!userId) return ResponseUtils.unauthorized(res, 'Authentication required');
