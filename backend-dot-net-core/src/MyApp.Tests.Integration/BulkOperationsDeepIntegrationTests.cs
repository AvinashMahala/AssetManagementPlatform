...
public class BulkOperationsDeepIntegrationTests : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    public async Task RentCollectionCreatesTransactions()
    {
        ...
        var leaseResp = await client.PostAsJsonAsync("/api/leases", lease);
    ...
    public async Task PaymentsCreatePaymentsAndReceipts()
    {
        ...
        var txResp = await client.PostAsJsonAsync("/api/renttransactions", new { leaseId = createdLease!.Id, amount = 700 });
        ...
        var payResp = await client.PostAsJsonAsync("/api/bulk/payments", payPayload);
