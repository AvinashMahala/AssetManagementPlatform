#!/usr/bin/env python3
"""
Database Cleanup Script for Expense Data - Asset Management Platform
Clears expense-related data while preserving other data.
"""
...
def clear_expense_data(conn):
    """Clear expense-related data from the database"""
    print_step("Clearing expense-related data...")
    ...
    # Print summary
    print_success("Expense-related data cleared successfully!")
...
def main():
    """Main function"""
    print_step("Asset Management Platform - Expense Data Cleanup")
    print("This script will clear all expense-related data from the database.")
    print("⚠️  WARNING: This action cannot be undone!")
    print()

    # Ask for confirmation
    if not ask_confirmation("Are you sure you want to clear all expense data?"):
    ...
    try:
        # Clear expense data
        if not clear_expense_data(conn):
            print_error("Failed to clear expense data")
            sys.exit(1)
        ...
        print_success("Expense data cleanup completed successfully!")
        print_info("You can now run the expense seeding script to populate fresh data.")
