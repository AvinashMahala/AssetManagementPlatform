#!/usr/bin/env python3
"""
Database Cleanup Script for Lease Data - Asset Management Platform
Clears lease-related data while preserving other data.
"""
...
def clear_lease_data(conn):
    """Clear lease-related data from the database"""
    print_step("Clearing lease-related data...")
    ...
    # Print summary
    print_success("Lease-related data cleared successfully!")
...
def main():
    """Main function"""
    print_step("Asset Management Platform - Lease Data Cleanup")
    print("This script will clear all lease-related data from the database.")
    print("⚠️  WARNING: This action cannot be undone!")
    print()

    # Ask for confirmation
    if not ask_confirmation("Are you sure you want to clear all lease data?"):
    ...
    try:
        # Clear lease data
        if not clear_lease_data(conn):
            print_error("Failed to clear lease data")
            sys.exit(1)
        ...
        print_success("Lease data cleanup completed successfully!")
        print_info("You can now run the lease seeding script to populate fresh data.")
