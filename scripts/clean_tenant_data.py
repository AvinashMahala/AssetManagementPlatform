#!/usr/bin/env python3
"""
Database Cleanup Script for Tenant Data - Asset Management Platform
Clears tenant-related data while preserving other data.
"""
...
def clear_tenant_data(conn):
    """Clear tenant-related data from the database"""
    print_step("Clearing tenant-related data...")
    ...
    # Print summary
    print_success("Tenant-related data cleared successfully!")
...
def main():
    """Main function"""
    print_step("Asset Management Platform - Tenant Data Cleanup")
    print("This script will clear all tenant-related data from the database.")
    print("⚠️  WARNING: This action cannot be undone!")
    print()

    # Ask for confirmation
    if not ask_confirmation("Are you sure you want to clear all tenant data?"):
    ...
    try:
        # Clear tenant data
        if not clear_tenant_data(conn):
            print_error("Failed to clear tenant data")
            sys.exit(1)
        ...
        print_success("Tenant data cleanup completed successfully!")
        print_info("You can now run the tenant seeding script to populate fresh data.")
